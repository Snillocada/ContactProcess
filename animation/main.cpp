...
//    int dir {mkdir(dir_name)};
    ...
    oss_aver <<"./runs/"<< direc_name<<"/data"<<"/locations.csv";
...
//    cout<<lambda_vec.size()<<endl;
    ...
    //        cout<<i;
            ...
            set<vector<long int>> particle_locations = particle_list.at(0)->get_particle_locations();
	    for(auto coords: particle_locations){
		aver_file<<coords.at(0)<<",";
	    }
	    aver_file<<endl;
	    for(auto coords_again: particle_locations) {
		aver_file<<coords_again.at(1)<<",";
	    }
	    aver_file<<endl;
